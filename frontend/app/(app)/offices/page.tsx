...
export default function OfficesPage() {
  const summaries = useAsync(() => meApi.officeSummaries(), []);
  const history = useAsync(() => meApi.officeHistory(), []);
            ...
            return (
              <Card key={office.id} className="flex flex-col">
                ...
                <div className="mt-auto flex flex-wrap gap-2 pt-4">
                  <Link href={`/offices/${office.code}`}>
                    <Button size="sm" variant="secondary">
                      Details
                    </Button>
                  </Link>
                  <Link href={`/offices/${office.code}?request=1`}>
                    <Button size="sm">Get a ticket</Button>
                  </Link>
                  {office.building_code ? (
                    <Link href={`/navigate?to=${encodeURIComponent(office.room_code ?? office.building_code)}`}>
