...
export default function TimetablePage() {
  ...
  const timetable = useAsync(() => meApi.timetable(weekStart), [weekStart]);
              ...
              <div key={date} className={`relative border-r border-ink-50 last:border-r-0 ${date === today ? 'bg-brand-50/30' : ''}`}>
                ...
                {(entriesByDate.get(date) ?? []).map((entry) => {
                  ...
                  return (
                    <Link
                      key={entry.id}
                      href={entry.room_code ? `/rooms/${entry.room_code}` : '/timetable'}
                ...
                <ul className="space-y-2">
                  {entries.map((entry) => (
                    <li key={entry.id} className="flex flex-wrap items-center gap-3 rounded-[12px] border border-ink-100 px-4 py-3">
                      ...
                      {entry.room_code ? (
                        <Link href={`/navigate?to=${encodeURIComponent(entry.room_code)}`}>
                          <Button variant="secondary" size="sm">
                            Directions
                          </Button>
                        </Link>
