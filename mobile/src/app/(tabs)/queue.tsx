...
import { ApiError, meApi, queueApi } from '@/lib/api';
...
export default function QueueScreen() {
  ...
  const ticket = useLoader(() => meApi.queueTicket(), []);
          ...
          return (
            <Card key={queue.id} style={{ marginBottom: spacing.md }}>
              ...
              <Small style={{ marginTop: spacing.sm }}>
                ...
                {queue.opens_at && queue.closes_at ? ` · ${queue.opens_at.slice(0, 5)}–${queue.closes_at.slice(0, 5)}` : ''}
              </Small>
              <Button
