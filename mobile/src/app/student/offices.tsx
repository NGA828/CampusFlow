...
export default function OfficesScreen() {
  ...
  // The desk list carries each caller's own live ticket, so the banner and the list cannot disagree.
  const myTicket = offices.data?.offices.find((summary) => summary.my_ticket)?.my_ticket ?? null;
      ...
      {myTicket ? (
        <View style={styles.padded}>
          <Card style={{ borderColor: colors.signal400, marginBottom: spacing.lg }}>
            ...
            <Button label="Open my ticket" variant="secondary" onPress={() => router.push(`/student/office/${myTicket.office.code}` as any)} style={{ marginTop: spacing.md }} />
          </Card>
        </View>
      ) : null}
      ...
      <View style={styles.padded}>
        {offices.data?.offices.map((summary) => (
          <Card key={summary.office.id} style={{ marginBottom: spacing.md }}>
            ...
            <Button label="Request a ticket" onPress={() => router.push(`/student/office/${summary.office.code}` as any)} style={{ marginTop: spacing.md }} />
          </Card>
        ))}
