...
import { assistantApi, meApi } from '../../../lib/api/endpoints';
...
export default function AssistantPage() {
  ...
  const tools = useAsync(() => assistantApi.tools(), []);
    ...
    <div>
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_300px]">
        ...
        <div className="space-y-4">
          ...
          <Card>
            <SectionHeading title="What the assistant can access" description={`${tools.data?.tools.length ?? 0} authorised tools`} />
            <ul className="space-y-1.5">
              {(tools.data?.tools ?? []).slice(0, 10).map((tool) => (
                <li key={tool.name} className="flex items-start justify-between gap-2 text-[12px]">
                  <span className="min-w-0">
                    <span className="block truncate font-medium text-ink-700">{tool.name.replaceAll('_', ' ')}</span>
                    <span className="block truncate text-ink-400">{tool.description}</span>
                  </span>
                </li>
              ))}
            </ul>
            <p className="mt-3 text-[11.5px] leading-relaxed text-ink-400">
              Tools are filtered by your role, so a student can never read staff or administration data through the assistant.
            </p>
          </Card>
        </div>
      </div>
