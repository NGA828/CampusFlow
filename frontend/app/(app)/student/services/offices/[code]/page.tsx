...
import { officeApi, studentApi } from '@/lib/api/endpoints';
...
/** Best-effort browser position; `null` when the student declines, which the server treats as "not verified". */
function readBrowserPosition(): Promise<{ lat: number; lng: number; accuracy_m?: number; source: string } | undefined> {
  if (typeof navigator === 'undefined' || !('geolocation' in navigator)) return Promise.resolve(undefined);
...
function OfficeDetail({ code }: { code: string }) {
  ...
  const active = useAsync(() => studentApi.activeOfficeTicket(), []);
  ...
  const request = async () => {
    ...
    setFormError(null);
    try {
      // Some desks refuse to issue a ticket to someone who is not standing there. The office's own
      // configuration says so, and the browser's geolocation is a legitimate answer on the web — the
      // server still decides whether the fix is close enough.
      const fix = office.data.office.requires_proximity_to_request ? await readBrowserPosition() : undefined;
      const issued = await officeApi.request(office.data.office.id, { subject: subject.trim(), notes: notes.trim() || undefined, fix });
      setTicket(issued);
      setShowForm(false);
      toast.success(`Ticket ${issued.ticket.ticket_number} requested`, 'We will call you when the office is ready.');
      router.replace(`/student/services/offices/${code}`);
    } catch (caught) {
  ...
  if (office.error) {
    return (
      <div>
        <PageHeader title="Office" breadcrumb={[{ label: 'Offices', href: '/student/services/offices' }, { label: code }]} />
  ...
  return (
    <div>
      <PageHeader
        ...
        breadcrumb={[{ label: 'Offices', href: '/student/services/offices' }, { label: details.code }]}
        actions={
          <>
            {details.room_code || details.building_code ? (
              <Link href={`/student/campus/map?route=${encodeURIComponent(details.room_code ?? details.building_code ?? '')}`}>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="space-y-4 lg:col-span-2">
          {ticket ? (
            ...
            <Card>
              ...
              <div className="mt-4 flex gap-2">
                ...
                {details.room_code ? (
                  <Link href={`/student/campus/map?route=${encodeURIComponent(details.room_code)}`}>
          ...
          <Card>
            <SectionHeading title="Queue right now" description={`${summary.today_in_line?.length ?? 0} tickets issued today`} />
            {(summary.today_in_line?.length ?? 0) === 0 ? (
